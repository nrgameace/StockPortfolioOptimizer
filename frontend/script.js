document.addEventListener("DOMContentLoaded", function () {
    ...
    const API_URL = "http://54.211.60.10:80/submit-portfolio";
    ...
    myButton.addEventListener("click", function (event) {
        ...
        if (!tickerData || isNaN(initialBudget.value) || tickerData.length < 3) {
            outputParagraph.textContent =
                "Please enter three or more stock tickers. Comma separated with no spaces.";
            return;
        }

        if (tickerData.length > modelList.length) {
            outputParagraph.textContent =
                "Ticker list has exceeded the number of avaliable stocks.";
            return;
        }

        if (!validData) {
            outputParagraph.textContent =
                "One of the tickers entered above is not in the avaliable list above. Please enter a stock symbol from above.";
            return;
        }
        ...
        fetch(API_URL, {
            ...
            .then((response) => {
                if (!response.ok) {
                    throw new Error(
                        `HTTP Error! Status: ${response.status}. Check console for details.`
                    );
                }
                // If success re
            ...
            .then((data) => {
                ...
                const weightsDisplay = data.tickers
                    .map((ticker, index) => {
                ...
                outputParagraph.innerHTML = `**Success!** Data processed.<br>
                                         ...
                                         **Budget:** $${data.initial_value.toFixed(
                    2
                )}<br>
                                         <br><strong>Portfolio Weights:</strong><br>
                                         ${weightsDisplay}
                                         <br><br>Expected Portfolio Return: $${data.expected_return
                    }</br></br>
                                         Expected Portfolio Variance: ${data.expected_variance.toFixed(
                        5
                    )}`;
